...
class ClusteringService:
    ...
    def get_all_clusters_with_metadata(self) -> List[dict]:
        """
        Get all clusters with metadata, INCLUDING NAMES.
        """
        try:
            all_faces = self.vectordb.collection.get(include=["metadatas"])
            clusters_dict = {}
            
            for metadata in all_faces.get("metadatas", []):
                ...
                if not clusters_dict[cluster_id]["name"] and metadata.get("name"):
                    clusters_dict[cluster_id]["name"] = metadata.get("name")
    ...
    def update_cluster_name(self, cluster_id: str, name: str) -> bool:
        try:
            faces = self.vectordb.get_faces_by_cluster(cluster_id)
            
            if not faces:
                logger.warning(f"Cannot name cluster {cluster_id}: Cluster not found or empty")
                return False
            ...
            for face in faces:
                ...
                current_meta = face.get("metadata", {}).copy()
                current_meta["name"] = name
                current_meta["cluster_id"] = cluster_id # Ensure this stays
                metadatas.append(current_meta)

            self.vectordb.collection.update(
                ids=face_ids,
                metadatas=metadatas
            )
            
            logger.info(f"Updated name to '{name}' for cluster {cluster_id} ({len(face_ids)} faces)")
            return True

        except Exception as e:
            logger.error(f"Error updating cluster name: {e}")
            raise
